...
from datetime import datetime as dt # For parsing dates
...
DATABASE_URL = os.environ.get('DATABASE_URL', 'sqlite:///invoices_pro.db')
# Generate a strong secret key for production (e.g., using os.urandom(24).hex())
# and set it as an environment variable. Provide a default for local dev.
SECRET_KEY = os.environ.get('SECRET_KEY', 'a-default-fallback-secret-key-change-me')

app.config['SQLALCHEMY_DATABASE_URI'] = DATABASE_URL
app.config['SECRET_KEY'] = SECRET_KEY
# Optional: Turn off SQLAlchemy modification tracking for performance
...
# -----------------------------
# (Your CompanyProfile, Customer, Invoice, InvoiceItem models are here - unchanged)
...
class Invoice(db.Model):
    ...
    date = db.Column(db.String(50), nullable=False) # We will store date as string
    po_number = db.Column(db.String(100))
    subtotal = db.Column(db.Float, nullable=False)
    total_gst = db.Column(db.Float, nullable=False)
    grand_total = db.Column(db.Float, nullable=False)
    po_date = db.Column(db.String(50)) # Storing as string
...
class InvoiceItem(db.Model):
    ...
    unit = db.Column(db.String(50)) # e.g., "KGS", "PCS", "LTR"
    invoice_id = db.Column(db.Integer, db.ForeignKey('invoice.id'), nullable=False)


# Create tables if they don't exist
# Note: For production, using migrations (Flask-Migrate) is better
...
# -----------------------------
# (get_currency_symbol, format_date, inject_helpers are here - unchanged)
def get_currency_symbol(currency_code):
    symbols = {'USD': '$', 'INR': '₹', 'EUR': '€', 'GBP': '£', 'JPY': '¥'}
    return symbols.get(currency_code, '$') # Default to $

def format_date(date_str, format_str):
    if not date_str: return 'N/A'
    try:
        date_obj = dt.strptime(date_str, '%Y-%m-%d')
        if format_str == 'DD/MM/YYYY': return date_obj.strftime('%d/%m/%Y')
        if format_str == 'MM/DD/YYYY': return date_obj.strftime('%m/%d/%Y')
        if format_str == 'YYYY-MM-DD': return date_obj.strftime('%Y-%m-%d')
    except ValueError: return date_str
    return date_str

@app.context_processor
def inject_helpers():
    return dict(get_currency_symbol=get_currency_symbol, format_date=format_date)

# -----------------------------
# ALL ROUTES (Unchanged)
# -----------------------------
# (home, dashboard, profile, customer_management, add_customer, edit_customer, delete_customer, add_invoice, view_invoice routes are here - unchanged)
@app.route('/')
def home():
...
def dashboard():
    ...
    # On Render, the first time after deploy, profile might not exist yet.
    # We still need it for the helpers, so handle this potentially.
    # However, create_all should handle the table, let's keep the redirect.
    if not profile:
         # Maybe redirect to profile to force creation, even in production?
         # Or handle gracefully in template if profile is None.
         # For simplicity, let's keep the redirect.
        flash("Welcome! Please create your company profile to get started.", "info")
        return redirect(url_for('profile'))

    invoices = Invoice.query.order_by(Invoice.date.desc()).all()
    # Pass profile even if None, template needs to handle it if forced creation fails.
...
def profile():
    ...
    if request.method == 'POST':
        if profile:
            ...
            profile.gstin = request.form['gstin']
            profile.pan = request.form['pan']
            profile.email = request.form['email']
            profile.phone = request.form['phone']
            profile.bank_name = request.form['bank_name']
            profile.account_no = request.form['account_no']
            profile.ifsc_code = request.form['ifsc_code']
            profile.currency = request.form['currency']
            profile.date_format = request.form['date_format']
            profile.terms_and_conditions = request.form['terms_and_conditions']
        ...
        db.session.commit()
        return redirect(url_for('profile'))
    # Pass profile (even if None) to the template
    return render_template('profile.html', profile=profile)


@app.route('/customers')
def customer_management():
    # Add check: Requires profile to exist before managing customers?
    profile = CompanyProfile.query.first()
    if not profile:
...
def add_customer():
    profile = CompanyProfile.query.first()
    if not profile:
        return redirect(url_for('profile')) # Ensure profile exists
...
def delete_customer(customer_id):
    ...
    if customer.invoices:
        flash(f"Cannot delete '{customer.name}'. It is linked to existing invoices.", "error")
    else:
...
def add_invoice():
    ...
    if request.method == 'POST':
        invoice_no_from_form = request.form['invoice_no']
        customer_id = request.form.get('customer')

        existing_invoice = Invoice.query.filter_by(invoice_no=invoice_no_from_form).first()
        if existing_invoice:
            flash(f"Error: Invoice number '{invoice_no_from_form}' already exists. Please use a different number.", "error")
        ...
        # Need to handle potential errors during DB operations in production
        try:
            ...
            # Log the error e
            return render_template( # Re-render form with error
                'invoice_form.html',
                customers=customers, profile=profile, today=today,
                invoice_data=request.form,
                selected_customer=int(customer_id) if customer_id else None
            )
        ...
        for i in range(len(item_names)):
            try:
                ...
                line_total = qty * rate
                gst_amount = line_total * (tax_percent / 100.0)
                subtotal_calc += line_total
                total_gst_calc += gst_amount
                new_item = InvoiceItem(
                    ...
                    invoice_id = new_invoice.id # ID is available after first commit
                )
                items_to_add.append(new_item)
            except (ValueError, IndexError) as e:
                # Log error or potentially inform user about bad item row i
                print(f"Skipping bad item row {i}: {e}")
                continue

        # Add all items and update totals in one transaction
        try:
            db.session.add_all(items_to_add)
            new_invoice.subtotal = subtotal_calc
            new_invoice.total_gst = total_gst_calc
            new_invoice.grand_total = subtotal_calc + total_gst_calc
            db.session.commit()
            flash(f"Invoice {new_invoice.invoice_no} created successfully!", "success")
            return redirect(url_for('dashboard'))
        except Exception as e:
            db.session.rollback()
            # If item saving fails, we might want to delete the invoice header
            # For simplicity now, just flash error
            flash(f"Error saving invoice items: {e}", "error")
            # Log the error e
            return render_template( # Re-render form with error
                'invoice_form.html',
                customers=customers, profile=profile, today=today,
...
def view_invoice(invoice_id):
    ...
    invoice = Invoice.query.filter_by(id=invoice_id).first_or_404() # Check ownership if users added
...
def download_invoice_pdf(invoice_id):
    profile = CompanyProfile.query.first()
    if not profile:
        flash("Cannot generate PDF without a company profile.", "warning")
        return redirect(url_for('profile')) # Redirect if no profile
    ...
    try:
        ...
        print("Successfully registered Arial font for PDF.")
    except Exception as e:
        print(f"WARNING: Could not register Arial font. Using default Helvetica. Rupee symbol might not display. Error: {e}")

    # --- Styles ---
    styles = getSampleStyleSheet()
    theme_color = colors.HexColor('#4A90E2')
    ...
    # --- Transport Table ---
    transport_data = [[Paragraph(f"<b>P.O. Number:</b><br/>{invoice.po_number or 'N/A'}", style_small), Paragraph(f"<b>P.O. Date:</b><br/>{format_date_pdf(invoice.po_date)}", style_small), Paragraph(f"<b>Transport:</b><br/>{invoice.transport_name or 'N/A'}", style_small), Paragraph(f"<b>Vehicle No.:</b><br/>{invoice.vehicle_no or 'N/A'}", style_small), Paragraph(f"<b>Delivery At:</b><br/>{invoice.delivery_location or 'N/A'}", style_small)]]
    ...
    for item in invoice.items:
        ...
        item_table_data.append([Paragraph(item.name, style_small), Paragraph(item.hsn or '', style_small), Paragraph(str(item.qty), style_small_right), Paragraph(item.unit or '', style_small_right), Paragraph(f"{currency}{item.rate:.2f}", style_small_right), Paragraph(f"{item.tax_percent:.0f}%", style_small_right), Paragraph(f"{currency}{line_total:.2f}", style_small_right)])
        tax_rate = item.tax_percent
        taxable_amount = line_total
        gst_amount = taxable_amount * (tax_rate / 100.0)
        if tax_rate in tax_summary: tax_summary[tax_rate]['taxable_amount'] += taxable_amount; tax_summary[tax_rate]['gst_amount'] += gst_amount
        else: tax_summary[tax_rate] = {'taxable_amount': taxable_amount, 'gst_amount': gst_amount}
    ...
    for rate, data in sorted(tax_summary.items()): tax_table_data.append([Paragraph(f"{rate:.0f}%", style_small), Paragraph(f"{currency}{data['taxable_amount']:.2f}", style_small_right), Paragraph(f"{currency}{data['gst_amount']:.2f}", style_small_right)])
    ...
    totals_data = [[Paragraph("Subtotal:", style_bold), Paragraph(f"{currency}{invoice.subtotal:.2f}", style_bold_right)], [Paragraph("Total GST:", style_bold), Paragraph(f"{currency}{invoice.total_gst:.2f}", style_bold_right)], [Paragraph("Grand Total:", style_bold), Paragraph(f"{currency}{invoice.grand_total:.2f}", style_bold_right)]]
    ...
    try:
        ...
        print(f"ERROR building PDF: {e}") # Log error during build
        flash(f"Error generating PDF: {e}", "error")
        return redirect(url_for('dashboard')) # Redirect on failure
...
# --- REMOVED DEVELOPMENT SERVER BLOCK ---
